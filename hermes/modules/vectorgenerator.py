...
class Vector(object):
	def __init__(self, sqlCtx, data, support_files):
		...
		get_vector_type = getattr(self, data.vector_type)
		if not get_vector_type:
			self.vector = None
		else:
			self.vector = get_vector_type()
...
class MovieLensContentVector(ContentVector, MovieLens):
	def genre(self):
		...
		return self.data.dataframe.map(lambda row: (row.movie_id, ))
...
class WikiUserVector(UserVector, Wiki):
	...
	def num_edits_ceil(self):
		return self.sqlCtx.sql("select user_id as user, article_id as item, count(1) as rating from wiki group by user_id, article_id")\
